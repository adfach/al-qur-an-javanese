...
import { surahs, sampleAyahs, Ayah } from '@/data/quranData';
import { useReadingPreferences } from '@/hooks/useLocalStorage';
import { cn } from '@/lib/utils';

// Generate sample ayahs for any surah
const generateAyahsForSurah = (surahNumber: number, totalAyahs: number): Ayah[] => {
  const surah = surahs.find(s => s.number === surahNumber);
  if (surahNumber === 1) return sampleAyahs;
  
  // For other surahs, generate placeholder ayahs
  return Array.from({ length: Math.min(totalAyahs, 10) }, (_, i) => ({
    number: i + 1,
    surahNumber,
    text: 'بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ',
    translation: `Terjemahan ayat ${i + 1} dari surah ${surah?.name || surahNumber}. (Data lengkap akan dimuat dari API)`,
    latin: `Transliterasi ayat ${i + 1}`,
    juz: surah?.juz[0] || 1,
    page: 1,
  }));
};
...
const SurahDetail: React.FC = () => {
  ...
  const ayahs = generateAyahsForSurah(surahNum, surah?.totalAyahs || 7);
          ...
          {surah.number !== 1 && (
            <div className="mt-8 p-4 rounded-xl bg-muted/50 border border-border text-center">
              <p className="text-sm text-muted-foreground">
                ℹ️ Data ayat lengkap akan dimuat dari API Al-Qur'an. 
                Saat ini menampilkan contoh tampilan untuk {Math.min(surah.totalAyahs, 10)} ayat pertama.
              </p>
            </div>
          )}
