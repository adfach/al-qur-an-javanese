  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {/* Stats Cards - 2 columns below Prayer Times */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8 animate-slide-up" style={{ animationDelay: '0.1s' }}>
            ...
            {lastReadSurah && (
              <Link
                ...
                className="group p-4 rounded-xl bg-accent/50 border border-accent flex items-center gap-4 hover:bg-accent/70 hover:shadow-lg transition-all duration-300 cursor-pointer"
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Ayat {preferences.lastReadAyah || 1} • Klik untuk lanjutkan
                  </p>
                </div>
            ...
            {/* Favorites Count - Clickable to expand */}
            {favoriteSurahs.length > 0 && (
              <button
                onClick={() => setShowFavorites(!showFavorites)}
                className="group p-4 rounded-xl bg-primary/10 border border-primary/20 flex items-center gap-4 hover:bg-primary/20 hover:shadow-lg transition-all duration-300 cursor-pointer text-left"
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-primary mt-1">
                    Klik untuk {showFavorites ? 'sembunyikan' : 'lihat daftar'}
                  </p>
                </div>
        ...
        {/* Favorites List - Expandable */}
        {showFavorites && favoriteSurahs.length > 0 && (
          <section className="mb-10 animate-slide-up">
            <div className="flex items-center gap-2 mb-4">
              <Heart className="w-5 h-5 text-primary fill-current" />
              <h3 className="text-lg font-semibold text-foreground">Daftar Surah Favorit Anda</h3>
              <span className="text-sm text-muted-foreground">({favoriteSurahs.length} surah)</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {favoriteSurahs.map((surah) => (
                <SurahCard key={surah.number} surah={surah} isFavorite />
              ))}
            </div>
          </section>
        )}
